...
\begin{remark}[Философия Рисса--Фреше]
	В линейных нормированных пространствах сопряженный оператор действует между
	сопряженными пространствами:
	\[
	A':H_2^*\to H_1^*,
	\qquad
	A'g=g\circ A.
	\]
	В гильбертовом пространстве по теореме Рисса--Фреше функционал можно записывать
	через скалярное произведение. Поэтому, если
	\[
	g(\xi)=(\xi,y)_{H_2},
	\]
	то
	...
	(Ax,y)_{H_2}.
	\]
	То есть гильбертово сопряжение является частным случаем обычного сопряжения после
	отождествления \(H\) и \(H^*\).
\end{remark}
